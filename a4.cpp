...
public:
	...
	void printChildren(){printChildren(root);};
};
...
void BinaryTree::countChildren(int num){
 	...
 	int count = 0;
 	if(nodePtr != NULL){
 		cout << "Parent node = " << nodePtr->value;
 		if(nodePtr->left == NULL){
 	...
 	if(count == 1){
 		cout << " has " << count << " child.\n";
 	} else {
 		cout << " has " << count << " children.\n";
 	}
...
void BinaryTree::printChildren(TreeNode *nodePtr){
	if (nodePtr){
		countChildren(nodePtr->value);
		printChildren(nodePtr->left);
		printChildren(nodePtr->right);
	}
}
